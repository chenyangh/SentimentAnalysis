    ...
    def __init__(self, embedding_mat, non_static, hidden_unit, sequence_length, max_pool_size,
        ...
        for i, filter_size in enumerate(filter_sizes):
            ...
            with tf.name_scope('conv2-maxpool-%s' % next_filter_size):
                W_2 = tf.Variable(tf.truncated_normal([next_filter_size, 1, num_filters, next_filter_num], stddev=0.1),
                                  name='W_2')
                b_2 = tf.Variable(tf.constant(0.1, shape=[next_filter_num]), name='b_2')
                conv2 = tf.nn.conv2d(h, W_2, strides=[1, 1, 1, 1], padding='SAME', name='conv2')
                ...
                reshaped = tf.reshape(h2, [-1, reduced3, next_filter_num])
                pooled_concat.append(reshaped)


        num_filters_total = next_filter_num * len(filter_sizes) * reduced3
        self.h_pool = tf.concat(2, pooled_concat)
        self.h_pool_flat = tf.reshape(self.h_pool, [-1, num_filters_total])
...
class TextCNNRNN2(object):
    def __init__(self, embedding_mat, non_static, hidden_unit, sequence_length, max_pool_size,
        ...
        self._initial_state = lstm_cell.zero_state(self.batch_size, tf.float32)
        inputs = [tf.squeeze(input_, [1]) for input_ in tf.split(1, reduced3, pooled_concat)]
        outputs, state = tf.nn.rnn(lstm_cell, inputs, initial_state=self._initial_state, sequence_length=self.real_len)
...
class TextCNNBiRNN(object):
    def __init__(self, embedding_mat, non_static, hidden_unit, sequence_length, max_pool_size,
        ...
        self._initial_state = lstm_cell.zero_state(self.batch_size, tf.float32)
        self._initial_state_bw = lstm_bw_cell.zero_state(self.batch_size, tf.float32)

        inputs = [tf.squeeze(input_, [1]) for input_ in tf.split(1, reduced, pooled_concat)]
        outputs, _, _ = tf.nn.bidirectional_rnn(lstm_cell, lstm_bw_cell, inputs, initial_state_fw=self._initial_state,
                                                initial_state_bw=self._initial_state_bw, sequence_length=self.real_len)
...
class TextCNNRNN2Bi(object):
    def __init__(self, embedding_mat, non_static, hidden_unit, sequence_length, max_pool_size,
        ...
        # lstm_cell = tf.nn.rnn_cell.LSTMCell(num_units=hidden_unit)
        lstm_cell = tf.nn.rnn_cell.GRUCell(num_units=int(hidden_unit / 2))
        lstm_cell = tf.nn.rnn_cell.DropoutWrapper(lstm_cell, output_keep_prob=self.dropout_keep_prob)
        lstm_bw_cell = tf.nn.rnn_cell.GRUCell(num_units=int(hidden_unit / 2))
        lstm_bw_cell = tf.nn.rnn_cell.DropoutWrapper(lstm_bw_cell, output_keep_prob=self.dropout_keep_prob)
        self._initial_state = lstm_cell.zero_state(self.batch_size, tf.float32)
        self._initial_state_bw = lstm_bw_cell.zero_state(self.batch_size, tf.float32)
